...
# compile + run the model (in ORT by default); verify with the oracle under `nnsmith_output`;
 offcom.model_exec model.path=nnsmith_output/model-b.onnx
# compile + run the model; verify with the oracle with fallback mode (random input)
 offcom.model_exec model.path=nnsmith_output/model-b.onnx cmp.oracle=null
# differential testing with tvm
 offcom.model_exec model.path=nnsmith_output/model-b.onnx cmp.with='{type:tvm, optmax:true, target:cpu}'
"""
...
from offcom.logging import EXEC_LOG
from offcom.macro import NNSMITH_BUG_PATTERN_TOKEN
from offcom.materialize import BugReport, Model, Oracle, TestCase

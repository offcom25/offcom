...
from offcom.abstract.extension import activate_ext
from offcom.backends.factory import BackendFactory
from offcom.cli.model_exec import verify_testcase
from offcom.error import InternalError
from offcom.filter import FILTERS
from offcom.graph_gen import model_gen, count_op_types
from offcom.logging import FUZZ_LOG
from offcom.macro import NNSMITH_BUG_PATTERN_TOKEN
from offcom.materialize import Model, TestCase
from offcom.narrow_spec import auto_opset
from offcom.util import (
...
class FuzzingLoop:
    def __init__(
        ...
        #ysy: add a new config to control the number of testcases
        self.iter = cfg["fuzz"]["iter"]

    def make_testcase(self, seed) -> TestCase:
        mgen_cfg = self.cfg["mgen"]
        method = random.choice(["TorchFusion"])  #  , "symbolic"
        gen = model_gen(
            opset=self.opset,
            method=method,#mgen_cfg["method"],
            seed=seed,
            max_elem_per_tensor=mgen_cfg["max_elem_per_tensor"],
            max_nodes=mgen_cfg["max_nodes"],
            nodes_allocation=mgen_cfg["nodes_allocation"],
            timeout_ms=mgen_cfg["timeout_ms"],
            rank_choices=eval(mgen_cfg["rank_choices"]),
            dtype_choices=("float32",) # mgen_cfg["dtype_choices"],
        )
        ir = gen.make_concrete()

        count_op_types("/root/.cache/offcom/op_count.json", ir)

        model = self.ModelType.from_gir(ir)
        if self.cfg["debug"]["viz"]:
            model.attach_viz(ir)

        # model.dump_viz("./graph.png")
    ...
    def run(self):
        ...
        while self.status.n_testcases < self.iter:

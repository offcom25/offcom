...
from offcom.abstract.dtype import DTYPE_GEN_INTS
from offcom.abstract.op import *
from offcom.abstract.op import ConcreteOp
from offcom.materialize import framework_operator_impl
from offcom.materialize.torch.dialect import Flatten, Linear, TorchReduceSum
...
@operator_impl(SetTuple)
def forward_fn(op: SetTuple):
    def setTuple(args):
        ans = []
        for arg in args:
            ans.append(arg)
        ans = tuple(ans)
        return ans
    return lambda *args: setTuple(args)

@operator_impl(GetTupleElem)
def forward_fn(op:GetTupleElem):
    def getTuple(x):
        if(op.nidx >= len(x)):
            return x[0]
        else:
            return x[op.nidx]
    return lambda x: getTuple(x)

@operator_impl(TupleSplit)
def forward_fn(op:TupleSplit):
    # op.tgt_dim
    # op.split_block_number
    def inner_split(x):
        x_shape = x.shape
        split_size = x_shape[op.tgt_dim] // op.split_block_number
        return torch.split(x,split_size,op.tgt_dim)
    return lambda x: inner_split(x)
# @operator_impl(GetTupleElem)
# def forward_fn(op: GetTupleElem):
#     def get_tuple(x):
#         if op.nidx >= len(x):

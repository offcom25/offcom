...
class Ploter:
    ...
    def plot(self, save="cov", cov_lim=None, loc=0):
        ...
        if cov_lim is not None:
            ...
            print(f"max_cov_unscale = {max_cov_unscale}")
            print(f"cov_lim = {cov_lim}")
            # self.axs[0].annotate(
...
def tvm_pass_filter(fname):
    ...
    #     return True
    # if "transform" in fname and "fuse" in fname:
    #     with open("./transform_fuse_file.txt", "a+") as f:
    #         f.write(fname+"\n")
    #     return True
    if "relay/transforms/fuse_ops.cc" in fname:
        # with open("./fuse_file.txt", "a+") as f:
        #     f.write(fname+"\n")
        return True
